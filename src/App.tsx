...
const router = createBrowserRouter([
  ...
  },
  {
    ...
    children: [
      ...
      { path: "judgment", element: <KeepAlive when={() => {const judementState = /^\/judgment\/[^/]+$/.test(location.pathname);return judementState;}}><JudgmentPage /></KeepAlive> },
      { path: "judgment/create", element: <CreateJudgment /> },
      { path: "judgment/:judgmentId", element: <JudgmentDetailPage /> },
      { path: "blame", element: <KeepAlive when={() => {const isBlameDetail = /^\/blame\/[^/]+$/.test(location.pathname);return isBlameDetail;}}><BlamePage /></KeepAlive> },
      { path: "blame/:blameId", element: <BlameDetailPage /> },
      { path: "persona", element:  <KeepAlive when={() => {const personaState = /^\/persona\/[^/]+$/.test(location.pathname);return personaState;}}><PersonaPage /></KeepAlive> },
      { path: "persona/create", element: <CreatePersona /> },
      { path: "persona/:personaId", element: <PersonaDetailPage /> },
      { path: "news", element: <KeepAlive when={() => {const newsState = /^\/news\/[^/]+$/.test(location.pathname);return newsState;}}><NewsPage /></KeepAlive> },
