...
const JudgmentVoteSelection = ({judgmentId,setTotalVoteCount}:JudgmentVoteSelection) =>{
    const {voteResult,totalCount, voted, vote}=useJudgmentVote(judgmentId);
    useEffect(()=>{
    ...
    return(
        <div className={style.judgmentVoteSelection}>
            {
                voted ? voteResult.map((result,index)=>(<SelectionBar key={index} title={result.selectionTitle} voteCount={result.voteCount} totalVoteCount={totalCount}/>))
